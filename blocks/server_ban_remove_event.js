module.exports = {
    ...
    description: "When an user is unbanned from a server, this event will trigger.",
    ...
    code(cache) {
        ...
        this.events.on("guildBanRemove", async (server, user) => {
            ...
            if(unstable_outputs == "yes" && server && server.me && server.me.hasPermission("VIEW_AUDIT_LOG")) {
                const entry = await server.fetchAuditLogs({type: "MEMBER_BAN_REMOVE", limit: 5}).then(audit => audit.entries.find(a => a.target.id == user.id && a.createdTimestamp >= current));
                if(entry) {
