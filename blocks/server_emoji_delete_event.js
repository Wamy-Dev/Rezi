module.exports = {
    ...
    description: "When an emoji is deleted in a server, this event will trigger.",
    ...
    code(cache) {
        ...
        this.events.on("emojiDelete", async server_emoji => {
            ...
            if(unstable_outputs == "yes" && server && server.me && server.me.hasPermission("VIEW_AUDIT_LOG")) {
                const entry = await server.fetchAuditLogs({type: "EMOJI_UPDATE", limit: 5}).then(audit => audit.entries.find(a => a.target.id == server_emoji.id && a.createdTimestamp >= current));
                if(entry) {
