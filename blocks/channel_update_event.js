    ...
    code(cache) {
        ...
        this.events.on("channelUpdate", async (old_channel, new_channel) => {
            ...
            if(unstable_outputs == "yes" && server && server.me && server.me.hasPermission("VIEW_AUDIT_LOG")) {
                const entry = await server.fetchAuditLogs({type: "CHANNEL_UPDATE", limit: 5}).then(audit => audit.entries.find(a => a.target.id == new_channel.id && a.createdTimestamp >= current));
                if(entry) {
