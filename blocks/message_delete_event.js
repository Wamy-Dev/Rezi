    ...
    code(cache) {
        ...
        this.events.on("messageDelete", async msg => {
            ...
            if(unstable_outputs == "yes" && server && server.me && server.me.hasPermission("VIEW_AUDIT_LOG")) {
                const entry = await server.fetchAuditLogs({type: "MESSAGE_DELETE", limit: 5}).then(audit => audit.entries.find(a => a.extra.channel.id == msg.channel.id && a.target.id == msg.author.id && a.createdTimestamp >= current));
                if(entry) {
