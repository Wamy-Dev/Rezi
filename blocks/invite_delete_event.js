module.exports = {
    ...
    description: "When an invite is deleted, this event will trigger. Your bot needs the permission \"Manage Server\" or \"Manage Channels\"!",
    ...
    code(cache) {
        ...
        this.events.on("inviteDelete", async invite => {
            ...
            if(unstable_outputs == "yes" && server && server.me && server.me.hasPermission("VIEW_AUDIT_LOG")) {
                const entry = await server.fetchAuditLogs({type: "INVITE_DELETE", limit: 5}).then(audit => audit.entries.find(a => a.target.code == invite.code && a.createdTimestamp >= current));
                if(entry) {
